...
export class Fish {
  ...
  /** Moon Seahorse — same anatomy as the others, with a bioluminescent glow. */
  _drawMoonSeahorse(g, sz, bodyColor, accentColor) {
    ...
    // Bony rings — glowing bands, narrowing toward tail, each with a dorsal spine
    const ringYs = [-0.30, -0.13, 0.03, 0.17, 0.30, 0.39, 0.46].map(t => t * bh);
    ringYs.forEach((ry, i) => {
      const taper = 1 - i / ringYs.length * 0.36;
      const xR =  bw * 0.50 * taper;
      const xL = -bw * 0.40 * taper;
      g.moveTo(xL, ry).lineTo(xR, ry)
       .stroke({ color: accentColor, width: 1.4, cap: 'round', alpha: 0.85 });
      g.moveTo(xL, ry)
       .lineTo(xL - sz * 0.09, ry - sz * 0.06)
       .stroke({ color: accentColor, width: 1.1, cap: 'round' });
    });
    ...
    g.stroke({ color: accentColor, width: 1.2, cap: 'round', join: 'round', alpha: 0.9 });
    ...
    // ── Coronet — 4 tapered spines with glowing knob tips ────────────────
    const crx = bw * 0.00, cry = -bh * 0.50;
    [
      [-sz * 0.10, -sz * 0.22],
      [-sz * 0.02, -sz * 0.27],
    ...
    // ── Glowing eye — iris ring + pupil + specular highlight ─────────────
    g.circle(bw * 0.26, -bh * 0.42, sz * 0.13).fill(accentColor);
    g.circle(bw * 0.26, -bh * 0.42, sz * 0.09).fill(0xffffff);
    g.circle(bw * 0.27, -bh * 0.415, sz * 0.065).fill(0x080820);
    g.circle(bw * 0.30, -bh * 0.428, sz * 0.025).fill(0xffffff);
  }
  ...
  _drawNautilus(g, sz, bodyColor, accentColor) {
    ...
    // ── Spiraling axial bands — taper outward, follow the whorl curl ──
    // Each band is a tapered polygon (thin at the umbilicus, fat at the
    // rim) sampled along a curve that bends with the shell's spiral.
    const STRIPES = 22;
    const baseA   = -Math.PI * 0.62;            // start ~11 o'clock
    const sweep   = Math.PI * 1.24;             // sweep CW to ~7 o'clock
    const curl    = -0.30;                      // bands bend CCW with the whorl
    const SAMPLES = 14;
    for (let i = 0; i < STRIPES; i++) {
      const t   = i / (STRIPES - 1);
      const j   = Math.sin(i * 1.7) * 0.018;    // angular jitter
      const a0  = baseA + (t + j) * sweep;
      const r0  = r * (0.20 + Math.abs(Math.sin(i * 0.9)) * 0.04);
      const r1  = r * (0.97 - Math.abs(t - 0.5) * 0.05);
      const wMax = sz * (0.10 + 0.05 * Math.sin(t * Math.PI) + Math.sin(i * 1.1) * 0.018);
      const wMin = wMax * 0.28;
      const col  = (i % 5 === 0) ? stripeLt : (i % 3 === 0) ? stripe : stripeDk;
      ...
      for (let s = 0; s <= SAMPLES; s++) {
        const u  = s / SAMPLES;
        const rs = r0 + (r1 - r0) * u;
        const as = a0 + curl * u * u;           // curl accelerates outward
        const px = Math.cos(as) * rs;
        const py = Math.sin(as) * rs;
        let tx, ty;
        if (s === 0) { tx = px; ty = py; }      // outward radial
        else { tx = px - pxPrev; ty = py - pyPrev; }
        const tlen = Math.hypot(tx, ty) || 1;
        const nx = -ty / tlen;
        const ny =  tx / tlen;
        const halfW = (wMin + (wMax - wMin) * u) * 0.5;
        left .push([px + nx * halfW, py + ny * halfW]);
        right.push([px - nx * halfW, py - ny * halfW]);
        pxPrev = px; pyPrev = py;
      }
      g.moveTo(left[0][0], left[0][1]);
      for (let s = 1; s < left.length; s++) g.lineTo(left[s][0], left[s][1]);
      for (let s = right.length - 1; s >= 0; s--) g.lineTo(right[s][0], right[s][1]);
      g.closePath().fill({ color: col, alpha: 0.82 });

      // Thin darker rib down the centerline of every other band, for depth
      if (i % 2 === 0) {
        let started = false;
        for (let s = 0; s <= SAMPLES; s++) {
          const u  = s / SAMPLES;
          const rs = r0 + (r1 - r0) * u;
          const as = a0 + curl * u * u;
          const px = Math.cos(as) * rs;
          const py = Math.sin(as) * rs;
          if (!started) { g.moveTo(px, py); started = true; }
          else g.lineTo(px, py);
        }
        g.stroke({ color: this._darken(col, 0.25), width: 0.6, alpha: 0.55 });
      }
    }
    ...
    // ── Inner whorl spiral — tighter, more emphatic log-spiral curl ──
    const PTS = 80;
    let started = false;
    for (let i = 0; i <= PTS; i++) {
      const t  = i / PTS;
      const a  = -Math.PI * 0.20 + t * Math.PI * 1.85;
      const rr = r * (0.16 + 0.78 * t);
      const x  = Math.cos(a) * rr;
      const y  = Math.sin(a) * rr;
      if (!started) { g.moveTo(x, y); started = true; }
      else g.lineTo(x, y);
    }
    g.stroke({ color: dark, width: 0.9, alpha: 0.30 });

    // Faint inner-whorl shadow — gives the illusion of overlapping coils
    let started2 = false;
    for (let i = 0; i <= PTS; i++) {
      const t  = i / PTS;
      const a  = -Math.PI * 0.20 + t * Math.PI * 1.85;
      const rr = r * (0.18 + 0.78 * t);
      const x  = Math.cos(a) * rr;
      const y  = Math.sin(a) * rr;
      if (!started2) { g.moveTo(x, y); started2 = true; }
      else g.lineTo(x, y);
    }
    g.stroke({ color: this._darken(cream, 0.14), width: 0.55, alpha: 0.45 });

    // Faint growth-line arcs near the outer rim (calcium lamellae)
    for (let i = 0; i < 4; i++) {
      const rr = r * (0.86 + i * 0.025);
      g.arc(0, 0, rr, -Math.PI * 0.55, Math.PI * 0.55)
       .stroke({ color: dark, width: 0.5, alpha: 0.20 });
    }
    ...
    // ── Pearly mother-of-pearl sheen near the aperture ───────────────
    g.circle(-r * 0.55, r * 0.42, r * 0.34)
     .fill({ color: 0xc8e4ff, alpha: 0.10 });
    g.circle(-r * 0.42, r * 0.30, r * 0.20)
     .fill({ color: 0xffe0ee, alpha: 0.09 });
    ...
    // Aperture inner shadow rim — depth illusion
    g.moveTo(-r * 0.18, r * 0.20)
     .bezierCurveTo(-r * 0.50, r * 0.10, -r * 0.92, r * 0.32, -r * 0.96, r * 0.55)
     .stroke({ color: this._darken(cream, 0.50), width: 1.1, alpha: 0.55 });
    ...
    // Hood ridge — soft warm highlight (less stark than a dark line)
    g.moveTo(-r * 0.28, r * 0.12)
     .bezierCurveTo(-r * 0.62, -r * 0.04, -r * 1.02, r * 0.06, -r * 1.08, r * 0.38)
     .stroke({ color: this._lighten(hood, 0.22), width: 1.0, alpha: 0.55 });
    ...
    // ── Cute face — huge sparkly eye, rosy cheeks, big smile ───────────
    const ex  = -r * 0.72;
    const ey  = r * 0.34;
    const eyR = sz * 0.22;                       // big, baby-like eye
    // Soft pale face patch so the features pop off the hood
    g.circle(ex + eyR * 0.2, ey + eyR * 0.2, eyR * 1.7).fill({ color: cream, alpha: 0.45 });
    // Eye white + dark border
    g.circle(ex, ey, eyR).fill(0xffffff);
    g.circle(ex, ey, eyR).stroke({ color: 0x2a1810, width: 1.2, alpha: 0.7 });
    // Big round pupil (fills most of the eye = cuter)
    g.circle(ex, ey + eyR * 0.10, eyR * 0.82).fill(0x241410);
    // Two glossy catchlights
    g.circle(ex - eyR * 0.28, ey - eyR * 0.30, eyR * 0.40).fill({ color: 0xffffff, alpha: 0.97 });
    g.circle(ex + eyR * 0.34, ey + eyR * 0.26, eyR * 0.20).fill({ color: 0xffffff, alpha: 0.85 });
    // Gentle eyelid arc
    g.moveTo(ex - eyR * 1.0, ey - eyR * 0.6)
     .quadraticCurveTo(ex, ey - eyR * 1.25, ex + eyR * 1.0, ey - eyR * 0.6)
     .stroke({ color: 0x2a1810, width: 1.2, alpha: 0.45 });
    // Rosy blush cheek
    g.circle(ex + eyR * 1.15, ey + eyR * 1.0, eyR * 0.55).fill({ color: 0xff8fa3, alpha: 0.5 });
    // Big happy smile (filled curve) with a tiny tongue
    const my = ey + eyR * 1.25;
    g.moveTo(ex - eyR * 0.55, my)
     .quadraticCurveTo(ex + eyR * 0.25, my + eyR * 0.95, ex + eyR * 1.05, my - eyR * 0.1)
     .stroke({ color: 0x6a2f24, width: 2, cap: 'round' });
    g.circle(ex + eyR * 0.25, my + eyR * 0.5, eyR * 0.18).fill({ color: 0xff7a8a, alpha: 0.85 });
  }
}
